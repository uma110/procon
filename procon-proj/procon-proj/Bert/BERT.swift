...
class BERT {
    ...
    func findAnswer(for question: String, in document: String) -> (answer:Substring,score:Double?) {
        ...
        guard let prediction = try? bertModel.prediction(input: modelInput) else {
            return ("The BERT model is unable to make a prediction.",nil)
        }
        ...
        guard let bestLogitIndices = bestLogitsIndices(from: prediction,
            ...
            return ("Couldn't find a valid answer. Please try again.",nil)
        }
        ...
        return (originalText[answerStart..<answerEnd],bestLogitIndices.score)
    }
}
